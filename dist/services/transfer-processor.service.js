...
export class TransferProcessorService {
    ...
    async processTransfer(transferId) {
        ...
        try {
            ...
            if (recipientMockToken) {
                ...
                const recipientAmount = await this.calculateRecipientAmount(transfer.sender_amount, transfer.sender_currency, transfer.recipient_currency);
                console.log(`💰 Calculated ${recipientMockToken} amount: ${recipientAmount.toString()} (${Number(recipientAmount) / 1000000} ${recipientMockToken})`);
     ...
     */
    async swapMockADAToRecipient(transfer, mockADAAmount, recipientMockToken, mintTxHash) {
        try {
     ...
     */
    async calculateRecipientAmount(sourceAmount, sourceCurrency, targetCurrency) {
        try {
            ...
            // Convert to token units with 6 decimals
            const tokenAmount = BigInt(Math.floor(targetAmount * 1000000));
            return tokenAmount;
        }
        catch (error) {
            ...
            return BigInt(Math.floor(targetAmount * 1000000));
        }
    }
    /**
     ...
     */
    async mintRecipientToken(transfer, recipientMockToken, amount) {
        try {
            console.log(`\n🪙 Minting ${recipientMockToken}...`);
            console.log(`   Source: ${transfer.sender_amount} ${transfer.sender_currency}`);
            console.log(`   Amount: ${amount.toString()} (${Number(amount) / 1000000} ${recipientMockToken})`);
            // Extract base symbol from mock token (e.g., mockIDRX → IDRX)
            const baseSymbol = recipientMockToken.replace('mock', '');
            const result = await this.cardanoService.mintToken({
                symbol: baseSymbol,
