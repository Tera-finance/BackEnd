...
exports.transferProcessorService = exports.TransferProcessorService = void 0;
const transfer_repository_1 = require("../repositories/transfer.repository");
const cardano_actions_service_1 = require("./cardano-actions.service");
const exchange_rate_api_service_1 = require("./exchange-rate-api.service");
...
class TransferProcessorService {
    constructor() {
        this.cardanoService = new cardano_actions_service_1.CardanoActionsService();
    }
    /**
    ...
    async processTransfer(transferId) {
        ...
        try {
            ...
            const mockADAAmount = await this.calculateMockADAAmount(transfer.sender_amount, transfer.sender_currency);
            console.log(`💰 Calculated mockADA amount: ${mockADAAmount.toString()} (${Number(mockADAAmount) / 1000000} ADA)`);
            ...
            await transfer_repository_1.TransferRepository.updateStatus(transferId, 'processing', {
            ...
            if (recipientMockToken) {
                ...
                const swapResult = await this.swapMockADAToRecipient(transfer, mockADAAmount, recipientMockToken);
                if (swapResult && swapResult.success) {
                    ...
                    await transfer_repository_1.TransferRepository.updateStatus(transferId, 'completed', {
            ...
            try {
                await transfer_repository_1.TransferRepository.updateStatus(transferId, 'failed');
            }
            catch (updateError) {
     ...
     */
    async mintMockADA(transfer, mockADAAmount) {
        try {
            console.log(`\n🪙 Minting mockADA...`);
            console.log(`   Source: ${transfer.sender_amount} ${transfer.sender_currency}`);
            console.log(`   Amount: ${mockADAAmount.toString()} (${Number(mockADAAmount) / 1000000} mockADA)`);
     ...
     */
    async swapMockADAToRecipient(transfer, mockADAAmount, recipientMockToken) {
        try {
            console.log(`\n🔄 Swapping mockADA to ${recipientMockToken}...`);
            // Get exchange rate: ADA → Recipient Currency
            const adaAmountDecimal = Number(mockADAAmount) / 1000000;
            const recipientAmount = await this.convertFromADA(adaAmountDecimal, transfer.recipient_currency);
            // Convert to token units (6 decimals)
            const recipientAmountBigInt = BigInt(Math.floor(recipientAmount * 1000000));
     ...
     */
    async calculateMockADAAmount(amount, sourceCurrency) {
        try {
            // Get conversion rate: Source Currency → ADA
            const adaAmount = await exchange_rate_api_service_1.exchangeRateService.convertToADA(amount, sourceCurrency);
            // Convert to token units with 6 decimals
            const mockADAWithDecimals = BigInt(Math.floor(adaAmount * 1000000));
            ...
            return BigInt(Math.floor(adaAmount * 1000000));
        }
    }
    /**
     * Convert ADA to target fiat currency
     */
    async convertFromADA(adaAmount, targetCurrency) {
        try {
            return await exchange_rate_api_service_1.exchangeRateService.convertFromADA(adaAmount, targetCurrency);
        }
        catch (error) {
     ...
     */
    getRecipientMockToken(currencyCode) {
        return (0, currencies_config_1.getMockToken)(currencyCode);
    }
}
exports.TransferProcessorService = TransferProcessorService;
// Export singleton instance
exports.transferProcessorService = new TransferProcessorService();
