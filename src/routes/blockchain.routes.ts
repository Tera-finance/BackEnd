 ...
 */
router.get('/balance/:tokenAddress', authenticate, async (req: Request, res: Response) => {
  try {
    const { tokenAddress } = req.params;
 ...
 */
router.post('/estimate-multi-swap', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/tx/:txHash/wait', async (req: Request, res: Response) => {
  try {
    const { txHash } = req.params;
