...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const transactions = await TransactionService.getCombinedHistory(
      req.user.id,
      limit,
      offset
    );
 ...
 */
router.get('/swaps', authenticate, async (req: Request, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({
        success: false,
    ...
    const swaps = await TransactionService.getSwapHistory(
      req.user.id,
      limit,
      offset
    );
 ...
 */
router.get('/hash/:txHash', authenticate, async (req: Request, res: Response) => {
  try {
