 ...
 */
export async function getTokenByPolicyId(policyId: string): Promise<CardanoToken | null> {
  return await queryOne<CardanoToken>(
    'SELECT * FROM cardano_tokens WHERE policy_id = ? AND is_active = true',
    [policyId]
  );
}
 ...
 * Get token by symbol
 */
export async function getTokenBySymbol(symbol: string): Promise<CardanoToken | null> {
  return await queryOne<CardanoToken>(
    'SELECT * FROM cardano_tokens WHERE token_symbol = ? AND is_active = true',
    [symbol]
  );
}
 ...
 */
export async function getAllActiveTokens(): Promise<CardanoToken[]> {
  return await query<CardanoToken>(
    'SELECT * FROM cardano_tokens WHERE is_active = true ORDER BY deployed_at DESC'
  );
}
 ...
 */
export async function updateTokenSupply(policyId: string, newSupply: bigint): Promise<void> {
  await query(
    'UPDATE cardano_tokens SET total_supply = ? WHERE policy_id = ?',
    [newSupply.toString(), policyId]
  );
}
...
export async function saveMintTransaction(mintData: {
  ...
  const token = await queryOne<CardanoToken>(
    'SELECT * FROM cardano_tokens WHERE id = ?',
    [tokenId]
  );
  ...
  const mint = await queryOne<CardanoMint>(
    'SELECT * FROM cardano_mints WHERE tx_hash = ?',
    [mintData.txHash]
  );
 ...
 */
export async function getMintHistory(policyId: string, limit = 10): Promise<CardanoMint[]> {
  const sql = `
  ...
  return await query<CardanoMint>(sql, [policyId, limit]);
}
...
export async function saveSwapTransaction(swapData: {
  ...
  if (!fromTokenId && swapData.fromPolicyId) {
    ...
    if (!token) throw new Error(`From token with policy ID ${swapData.fromPolicyId} not found`);
  ...
  if (!toTokenId && swapData.toPolicyId) {
    ...
    if (!token) throw new Error(`To token with policy ID ${swapData.toPolicyId} not found`);
    toTokenId = token.id;
  }
  ...
  if (swapData.swapType === 'VIA_HUB' && !hubTokenId && swapData.hubPolicyId) {
    ...
    if (!token) throw new Error(`Hub token with policy ID ${swapData.hubPolicyId} not found`);
    hubTokenId = token.id;
  }
  ...
  const swap = await queryOne<CardanoSwap>(
    'SELECT * FROM cardano_swaps WHERE tx_hash = ?',
    [swapData.txHash]
  );
 ...
 * Get swap history
 */
export async function getSwapHistory(limit = 10): Promise<CardanoSwap[]> {
  return await query<CardanoSwap>(
    'SELECT * FROM cardano_swaps ORDER BY created_at DESC LIMIT ?',
    [limit]
  );
}
 ...
 */
export async function getSwapHistoryByTokens(
  fromPolicyId?: string,
  toPolicyId?: string,
  limit = 10
): Promise<CardanoSwap[]> {
  ...
  return await query<CardanoSwap>(sql, params);
}
 ...
 */
export async function getTokenStats(policyId: string): Promise<{
  totalMints: number;
  ...
  const mintStats = await queryOne<any>(
    `SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total 
     FROM cardano_mints WHERE token_id = ?`,
    [token.id]
  );

  const swapStats = await queryOne<any>(
    `SELECT COUNT(*) as count FROM cardano_swaps 
     WHERE from_token_id = ? OR to_token_id = ?`,
    [token.id, token.id]
  );

  const lastActivity = await queryOne<any>(
